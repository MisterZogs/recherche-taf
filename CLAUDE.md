...
| **euremotejobs.com** | Contrairement à la note précédente ("curl + UA passe"), **403 confirmé aujourd'hui** avec le même UA navigateur standard — possible durcissement anti-bot depuis le 30/08, à retester avec un throttle plus lent |
...
**Contrôle du 01/09/2026 : les onglets actifs sont désormais entièrement propres.** Un nouveau balayage a trouvé 3 liens génériques résiduels dans les onglets actifs : le lien individuel a été retrouvé et vérifié (200) pour SAP Principal Solution Advisor HCM (`jobs.sap.com/job/Levallois-Perret-Principal-Solution-Advisor%2C-HCM-FM-92300/1286346301/`), et les 2 autres (Consultant SAP HCM Time chez PREREQUIS, Product Manager chez Follow) étaient introuvables, donc cellule Lien vidée et raison documentée dans Fit / Notes conformément à la règle. **Il reste ~89 liens génériques partagés dans NoRemote et Fait uniquement** (offres déjà écartées ou traitées, priorité faible) : c'est le reliquat de l'audit du 21/08, à reprendre au fil de l'eau.
...
Cette règle a remplacé le même jour une première version qui demandait de mettre systématiquement "À postuler" : Gaëtan a précisé vouloir une date à la place, plus informative qu'un simple mot-clé. Migration rétroactive faite le 09/09/2026 : sur 555 lignes portant "Nouveau" ou "À postuler", 256 ont pu être converties (dont `Date publiée` en priorité, `Date trouvée` sinon) ; **299 lignes plus anciennes n'ont ni l'une ni l'autre renseignée et ont été laissées inchangées** plutôt que de leur inventer une date. Ce reliquat est à reprendre au fil de l'eau si une de ces offres redevient pertinente (candidature, vérification de lien...), pas à corriger en masse avec une date arbitraire.
