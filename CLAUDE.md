...
#### Repasse de renotation manuelle du 16/09/2026 (en cours, à terminer)
...
**État au 16/09/2026 en fin de session (session coupée par l'utilisateur, pas par manque de travail) :**
- ✅ Terminés et intégrés : Offres UX+SEO (40 offres), Pays Basque+Offres USA (38), Offres PM (48), Offres IA (76), Offres SIRH (63), Offres CSM lot B (67). Total 332 lignes traitées et poussées sur `master`.
- ❌ **Reste à faire : Offres CSM, 67 lignes non revues** (le lot "A" du découpage en 3 tiers de l'onglet CSM ; l'agent a été tué en cours de route avant d'avoir rien écrit, aucune perte de travail réel puisqu'il n'avait encore rien produit). Pour reprendre : régénérer l'export avec le filtre `Priorité in (4,5) AND '[revu' not in Fit/Notes` sur l'onglet "Offres CSM" (67 lignes actuellement, voir requête dans `ajout_offres_relance_20260916.py`/session précédente), relancer un agent avec le même prompt type que les lots CSM A/B/C précédents (lire CLAUDE.md pour le contexte du profil, fetch chaque lien, statut Expiré si lien mort, note 1-5 étoiles réfléchie + fit concret sinon), puis appliquer avec `python3 appliquer_renotation.py <fichier>.json` et committer/pousser.
- Une session précédente a aussi révélé qu'un lot lancé sans écriture incrémentale peut tout perdre si la limite de session API est atteinte en cours de route ("You've hit your session limit") : toujours instruire l'agent d'écrire son fichier de résultats au fur et à mesure (toutes les 5-10 offres), jamais seulement à la fin.
